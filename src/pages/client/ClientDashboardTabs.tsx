        ...
        <div className="space-y-4">
          ...
          {brief?.status === "filled" && (
            <div className="bg-green-50 border border-green-200 rounded-2xl p-4 flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-green-800">Бриф заполнен</p>
                <p className="text-xs text-green-600 mt-0.5">Дизайнер уже получил ваши пожелания. Вы можете обновить их в любое время.</p>
              </div>
            </div>
          )}
          ...
          <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
            <div className="px-4 py-3 border-b border-gray-100">
              <p className="text-sm font-semibold text-gray-800">Бриф на дизайн-проект</p>
              <p className="text-xs text-gray-400 mt-0.5">Расскажите о своих пожеланиях — это поможет дизайнеру создать именно то, что вам нужно</p>
            </div>
            <div className="divide-y divide-gray-50">
              {([
                { key: "style",          label: "Стиль интерьера",        placeholder: "Современный, скандинавский, лофт, классика...", long: false },
                { key: "color_palette",  label: "Цветовая палитра",       placeholder: "Нейтральные тона, яркие акценты, любимые цвета...", long: true },
                { key: "furniture",      label: "Предпочтения по мебели", placeholder: "Материалы, стиль, есть ли уже мебель...", long: true },
                { key: "wishes",         label: "Пожелания и идеи",       placeholder: "Что важно, что нравится, чего хотите достичь...", long: true },
                { key: "restrictions",   label: "Ограничения",            placeholder: "Что точно не подходит, аллергии, конструктивные ограничения...", long: true },
                { key: "extra",          label: "Дополнительно",          placeholder: "Любая другая полезная информация...", long: true },
                { key: "client_comment", label: "Вопросы дизайнеру",      placeholder: "Уточнения, вопросы, пожелания по процессу...", long: true },
              ] as { key: string; label: string; placeholder: string; long: boolean }[]).map(f => (
                <div key={f.key} className="px-4 py-3">
                  ...
                  {f.long ? (
                    <textarea
                      ...
                      className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm placeholder:text-gray-300 focus:outline-none focus:border-gray-400 resize-none"
                    />
                  ) : (
                    <input
                      value={briefValues[f.key] ?? ""}
                      ...
                      className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm placeholder:text-gray-300 focus:outline-none focus:border-gray-400"
                    />
                  )}
                </div>
              ))}
            ...
            <div className="px-4 pb-4 pt-2">
              <button onClick={onSaveBrief} disabled={savingBrief}
                className="w-full py-3 bg-gray-900 text-white text-sm font-medium rounded-xl hover:bg-gray-800 disabled:opacity-50 flex items-center justify-center gap-2">
                {savingBrief
                  ? <div className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                  : <Icon name="Send" size={14} />}
                {savingBrief ? "Сохраняю..." : "Отправить дизайнеру"}
              </button>
            </div>
          </div>
        </div>
